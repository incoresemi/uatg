class regex_formats:
    ...
    def __init__(self):
        ...
        self.bpu_match_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sMatch\:\d{8}"

        # [ 5570] [ 0]BPU : BTB Hit: BTBEntry { target: 'h00000000800003ac,
        #                                        ci: Branch, instr16: False, hi: False }
        self.btb_hit_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sBTB\sHit\:\sBTBEntry" \
        ...
        self.pred_req_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sReceived\sRequest\:" \
        ...
        #                                                              , history: 'h00 }
        self.train_data_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sReceived\s" \
        ...
        self.alloc_newind_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sAllocating\snew\s" \
                                    + "index\:\s+[0-9]+\sghr\:\s\d+"

        # [      7880] [ 0]BPU : Conflict Detected
        self.conflict_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sConflict\sDetected"

        # [      6320] [ 0]BPU : Pushing into RAS:00000000800003c8
        self.pushing_to_ras_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sPushing\s" \
                                      + "into\sRAS\:[0-9a-z]+"

        # [      6540] [ 0]BPU : Choosing from top RAS:0000000080000404
        self.choosing_top_ras_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sChoosing\s" \
                                        + "from\stop\sRAS\:[0-9a-z]+"

        # [      6000] [ 0]BPU : New GHR: 01111100
        self.new_ghr_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sNew\sGHR\:\s\d+"

        # [      5920] [ 0]BPU : BHTindex_:233 Target:0000000080000396 Pred:3
        #                                                                  ghr: 11110000
        self.bht_ind_target_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\s" \
                                      + "BHTindex_\:\d+\sTarget\:\s*\d+\sPred\:\d+\sghr\:\s\d+"

        # [      7950] [ 0]BPU : Misprediction fired. Restoring ghr: 01111111
        self.misprediction_pattern = "\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\s" \
        ...
        """
        self.fence_executed_pattern = "(\[\s*[0-9]*\]\s\[\s*[0-9]*\]BPU\s\:\sFenced\,\s" \
